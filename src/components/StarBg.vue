    ...
    export default {
        ...
        methods: {
            initStar () {
                let context;// 整个上下文
                let arr = [];// 星星数组
                let starCount = 500; // 星星开始数量

                let rains = [];// 流星雨数组
                let rainCount = 20;// 流星雨的数量
                ...
                let MeteorRain = function () {
                    ...
                    this.init = function () {
                        ...
                        // 流星雨的最小长度，最大长度
                        let x = Math.random() * 40 + 250;
                        // 流星雨的长度
                        this.length = Math.ceil(x);// 对随机数进行上舍入
                        this.angle = 170;// 流星倾斜角
                        x = Math.random() + 0.5;
                ...
                window.onload = function () {
                    ...
                    // 画流星
                    for (let i = 0; i < rainCount; i++) {
                        let rain = new MeteorRain();// 创建一个流星对象
                        rain.init();// 调用流星初始化方法
                        rain.draw(); // 调用绘制流星方法
